        ...
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            {currentDisplayedData.map((item, i) => {
              return (
                <tr
                  key={item.id ?? i} // use `item.id` if available, fallback to `i`
                  className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600"
                >
                  <td className="w-4 p-4">
                    <div className="flex items-center">
                      <input
                        id={`checkbox-table-search-${item.id ?? i}`}
                        type="checkbox"
                        className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"
                      />
                      <label
                        htmlFor={`checkbox-table-search-${item.id ?? i}`}
                        className="sr-only"
                      >
                        checkbox
                      </label>
              ...
              return (
                <li key={index}>
                  <button
                    ...
                    className={
                      currentPage == index + 1
                        ? 'flex items-center justify-center px-3 h-10 leading-tight text-white bg-blue-600 border border-blue-300 hover:bg-gray-100 hover:text-white dark:bg-blue-800 dark:border-blue-700 dark:text-white dark:hover:bg-blue-700 dark:hover:text-white'
